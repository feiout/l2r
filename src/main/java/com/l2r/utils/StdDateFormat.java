...
public class StdDateFormat extends DateFormat {
	...
	/*
	 * TODO !!! 24-Nov-2009, tatu: Need to rewrite this class: JDK date parsing is awfully brittle, and ISO-8601 is quite permissive. The two don't mix, need to write a better one.
	 */

	/**
	 * 
	 */
	private static final long serialVersionUID = -5450404172129668439L;
	...
	protected final static String TIME_FORMAT_STR_HHmm = "HH:mm";
	protected final static String TIME_FORMAT_STR_HHmmss = "HH:mm:ss";

	protected final static String DATE_TIME_FORMAT_STR_HHmm = "yyyy-MM-dd HH:mm";
	protected final static String DATE_TIME_FORMAT_STR_HHmmss = "yyyy-MM-dd HH:mm:ss";
	...
	protected final static String[] ALL_FORMATS = new String[] { DATE_FORMAT_STR_ISO8601, DATE_FORMAT_STR_ISO8601_Z,
			DATE_FORMAT_STR_RFC1123, DATE_FORMAT_STR_PLAIN,TIME_FORMAT_STR_HHmm,TIME_FORMAT_STR_HHmmss,
			DATE_TIME_FORMAT_STR_HHmm,DATE_TIME_FORMAT_STR_HHmmss};
	...
	static {
		DEFAULT_TIMEZONE = TimeZone.getTimeZone("GMT+08:00");
	}
	...
	protected final static DateFormat TIME_FORMAT_HHmm;

	protected final static DateFormat TIME_FORMAT_HHmmss;

	protected final static DateFormat DATE_TIME_FORMAT_HHmm;

	protected final static DateFormat DATE_TIME_FORMAT_HHmmss;

	/*
	 * Let's construct "blueprint" date format instances: can not be used as is, due to thread-safety issues, but can be used for constructing actual instances more cheaply (avoids re-parsing).
	 */
	static {
		/*
		 * Another important thing: let's force use of GMT for baseline DataFormat objects
		 */
		...
		TIME_FORMAT_HHmm = new SimpleDateFormat(TIME_FORMAT_STR_HHmm, DEFAULT_LOCALE);
		TIME_FORMAT_HHmm.setTimeZone(DEFAULT_TIMEZONE);

		TIME_FORMAT_HHmmss = new SimpleDateFormat(TIME_FORMAT_STR_HHmmss, DEFAULT_LOCALE);
		TIME_FORMAT_HHmmss.setTimeZone(DEFAULT_TIMEZONE);

		DATE_TIME_FORMAT_HHmm = new SimpleDateFormat(DATE_TIME_FORMAT_STR_HHmm, DEFAULT_LOCALE);
		DATE_TIME_FORMAT_HHmm.setTimeZone(DEFAULT_TIMEZONE);
		DATE_TIME_FORMAT_HHmmss = new SimpleDateFormat(DATE_TIME_FORMAT_STR_HHmmss, DEFAULT_LOCALE);
		DATE_TIME_FORMAT_HHmmss.setTimeZone(DEFAULT_TIMEZONE);
	...
	protected transient DateFormat _formatHHmm;
	protected transient DateFormat _formatHHmmss;
	protected transient DateFormat _formatDateHHmm;
	protected transient DateFormat _formatDateHHmmss;
	...
	public StdDateFormat clone() {
		/*
		 * Although there is that much state to share, we do need to orchestrate a bit, mostly since timezones may be changed
		 */
	...
	public static DateFormat getHHmmFormat(TimeZone tz, Locale loc) {
		return _cloneFormat(TIME_FORMAT_HHmm, TIME_FORMAT_STR_HHmm, tz, loc);
	}

	public static DateFormat getHHmmssFormat(TimeZone tz, Locale loc) {
		return _cloneFormat(TIME_FORMAT_HHmmss, TIME_FORMAT_STR_HHmmss, tz, loc);
	}
	/**
	...
	public Date parse(String dateStr) throws ParseException {
		...
		if (dateStr.charAt(0) == '0' && dateStr.length() >= 10) {
			return null;
		} else {
	...
	public Date parse(String dateStr, ParsePosition pos) {
		...
		/*
		 * 14-Feb-2010, tatu: As per [JACKSON-236], better also consider "stringified" simple time stamp
		 */
	...
	protected boolean looksLikeISO8601(String dateStr) {
		if (dateStr.length() >= 5 && Character.isDigit(dateStr.charAt(0)) && Character.isDigit(dateStr.charAt(3)) && (dateStr.charAt(4) == '-' || dateStr.charAt(2) == ':')) {
			return true;
		}
		return false;
	}
	...
	protected Date parseAsISO8601(String dateStr, ParsePosition pos) {
		...
		/*
		 * First: do we have "zulu" format ('Z' == "GMT")? If yes, that's quite simple because we already set date format timezone to be GMT, and hence can just strip out 'Z' altogether
		 */
		...
		if (len <= 19 && Character.isDigit(c) && dateStr.lastIndexOf('T') < 0) {
			if (len == 19)
			{
				df = _formatDateHHmmss;
				if (df == null) {
					df = _formatDateHHmmss = _cloneFormat(DATE_TIME_FORMAT_HHmmss, DATE_TIME_FORMAT_STR_HHmmss, _timezone, _locale);
				}
			}
			else if (len == 16)
			{
				df = _formatDateHHmm;
				if (df == null) {
					df = _formatDateHHmm = _cloneFormat(DATE_TIME_FORMAT_HHmm, DATE_TIME_FORMAT_STR_HHmm, _timezone, _locale);
				}
			}
			else if (len == 8)
			{
				df = _formatHHmmss;
				if (df == null) {
					df = _formatHHmmss = _cloneFormat(TIME_FORMAT_HHmmss, TIME_FORMAT_STR_HHmmss, _timezone, _locale);
				}
			}
			else if (len == 5)
			{
				df = _formatHHmm;
				if (df == null) {
					df = _formatHHmm = _cloneFormat(TIME_FORMAT_HHmm, TIME_FORMAT_STR_HHmm, _timezone, _locale);
				}
			}
			else
			{
			...
			if (hasTimeZone(dateStr)) {
				...
				// [JACKSON-334]: may be missing milliseconds... if so, add
				len = dateStr.length();
				//'2014-08-06T10:16:28.1690726+08:00' :
				if (len > 28) {
					StringBuilder sb = new StringBuilder(dateStr);
					sb.delete(23,sb.lastIndexOf("+"));
					dateStr = sb.toString();
					len = dateStr.length();
				}
				...
				/*
				 * 24-Nov-2009, tatu: Ugh. This is getting pretty ugly. Need to rewrite!
				 */
